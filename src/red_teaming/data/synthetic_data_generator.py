...
class SyntheticDataGenerator:
    ...
    def generate_synthetic_text(self, original_text, preserve_patterns=True):
        ...
        if preserve_patterns:
            ...
            # Append marker without adding a new word
            if synthetic_words:
                synthetic_words[-1] = synthetic_words[-1] + "[SYNTHETIC]"
            return ' '.join(synthetic_words)
        else:
            # Simply generate random characters/words
            random_text = ''.join(random.choices(string.ascii_lowercase + string.digits, k=len(original_text)))
            return random_text + " [SYNTHETIC_RANDOM]"
